...
int main(int argc, char ** argv) {
   ...
   if (argc != 3) {
      printf("usage:\n%s [odrive0 tty] [odrive1 tty]\n",argv[0]);
      exit(1);
   }

   int conn0 = open(argv[1],O_RDWR | O_NOCTTY | O_NONBLOCK);
   if (errno != 0) {
      perror("connection 0");
      exit(1);
   }

   int conn1 = open(argv[2],O_RDWR | O_NOCTTY | O_NONBLOCK);
   if (errno != 0) {
      perror("connection 1");
      exit(1);
   }
   ...
   for ( ; ; ) {
      ...
      switch (key) {
         case 'w':
            ...
            count = nonblock_write(conn0,buf);

            snprintf(buf,128,"v 1 %f\n",conv(-speed));

            count = nonblock_write(conn1,buf);
